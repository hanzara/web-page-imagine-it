-- Create comprehensive admin loan oversight functionality

-- First, let's add some missing columns and functions for better loan tracking
DO $$ BEGIN
  -- Add region column to profiles if it doesn't exist
  ...
  -- Add credit_score column to profiles if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'credit_score') THEN
    ALTER TABLE public.profiles ADD COLUMN credit_score INTEGER DEFAULT 650;
  END IF;
  
  -- Add repayment_rate column to loan_applications_new if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'loan_applications_new' AND column_name = 'repayment_rate') THEN
    ALTER TABLE public.loan_applications_new ADD COLUMN repayment_rate NUMERIC DEFAULT 0;
  END IF;
  
  -- Add disbursed_amount column to loan_applications_new if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'loan_applications_new' AND column_name = 'disbursed_amount') THEN
    ALTER TABLE public.loan_applications_new ADD COLUMN disbursed_amount NUMERIC DEFAULT 0;
  END IF;
  
  -- Add due_date column to loan_applications_new if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'loan_applications_new' AND column_name = 'due_date') THEN
    ALTER TABLE public.loan_applications_new ADD COLUMN due_date DATE;
  END IF;
  
  -- Add repaid_amount column to loan_applications_new if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'loan_applications_new' AND column_name = 'repaid_amount') THEN
    ALTER TABLE public.loan_applications_new ADD COLUMN repaid_amount NUMERIC DEFAULT 0;
  END IF;
END $$;

-- Create a comprehensive function to get all loans for admin oversight
...
BEGIN
  ...
  SELECT 
    ...
    la.disbursed_amount,
    la.repaid_amount,
    (la.disbursed_amount - la.repaid_amount) as remaining_amount,
    la.interest_rate,
    la.loan_term_months,
    la.status,
    la.risk_rating,
    ...
    la.due_date,
    CASE 
      WHEN la.due_date IS NOT NULL AND la.due_date < CURRENT_DATE AND la.status = 'active' 
  ...
  SELECT 
    ...
    cl.repaid_amount,
    (cl.amount - cl.repaid_amount) as remaining_amount,
    cl.interest_rate,
    ...
    COALESCE(p.credit_score, 650) as credit_score,
    CASE 
      WHEN cl.amount > 0 THEN (cl.repaid_amount / cl.amount * 100)
      ELSE 0
    ...
    cl.due_date,
    CASE 
      WHEN cl.due_date IS NOT NULL AND cl.due_date < CURRENT_DATE AND cl.status = 'active' 
...
-- Create function to get loan statistics for admin dashboard
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(AVG(repayment_rate), 0) as avg_rate
  ...
  SELECT 
    ...
    COUNT(CASE WHEN status = 'active' AND due_date < CURRENT_DATE THEN 1 END) as overdue_count,
    COUNT(CASE WHEN status = 'completed' THEN 1 END) as completed_count,
    COALESCE(AVG(CASE WHEN amount > 0 THEN (repaid_amount / amount * 100) ELSE 0 END), 0) as avg_rate
  INTO chama_stats
  ...
  SELECT 
    ...
    jsonb_build_object(
      'low', (SELECT COUNT(*) FROM loan_applications_new WHERE risk_rating = 'low'),
      'medium', (SELECT COUNT(*) FROM loan_applications_new WHERE risk_rating = 'medium'),
      'high', (SELECT COUNT(*) FROM loan_applications_new WHERE risk_rating = 'high')
    ) as risk_distribution;
END;
$$;

-- Create function to approve/reject loan applications
...
CREATE POLICY "Admins can view all loan applications" ON public.loan_applications_new
FOR SELECT USING (
  EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'admin')
);

CREATE POLICY "Admins can update loan applications" ON public.loan_applications_new
FOR UPDATE USING (
  EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'admin')
);

-- Grant execute permissions to authenticated users
GRANT EXECUTE ON FUNCTION public.get_admin_loans_overview() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_admin_loan_statistics() TO authenticated;
