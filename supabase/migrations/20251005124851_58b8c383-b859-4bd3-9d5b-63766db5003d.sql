-- Drop and recreate both chama initialization triggers to fix ON CONFLICT issues

-- Drop existing triggers
DROP TRIGGER IF EXISTS create_chama_central_wallet_trigger_v2 ON public.chamas;
DROP TRIGGER IF EXISTS trigger_initialize_chama ON public.chamas;

-- Drop existing functions
DROP FUNCTION IF EXISTS public.create_chama_central_wallet_v2();
DROP FUNCTION IF EXISTS public.initialize_chama_on_creation();

-- Create a single comprehensive initialization function
...
-- Create the trigger (only one now)
CREATE TRIGGER trigger_initialize_chama
  AFTER INSERT ON public.chamas
  FOR EACH ROW
  EXECUTE FUNCTION public.initialize_chama_on_creation();