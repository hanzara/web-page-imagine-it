...
export const ChamaMembersTab: React.FC<ChamaMembersTabProps> = ({ 
  ...
  const { updateMemberRole, updateMemberName, removeMember } = useMemberManagement(chamaId);
    ...
    <div className="space-y-6">
      ...
      {selectedMember && (
        <MemberDetailsModal
          ...
          onChangeRole={(memberId, newRole) => updateMemberRole({ memberId, newRole })}
          onUpdateName={(userId, newName) => updateMemberName({ userId, newName })}
          isCurrentUser={user?.id === selectedMember.user_id}
        />
      )}
