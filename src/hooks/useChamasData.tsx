...
export const useUserChamas = () => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Combine both lists - only show chamas user owns or is a member of
      const memberChamasList = memberChamas.map(member => ({
        ...
        isOwned: member.chamas.purchased_by === user.id
      }));

      // Add purchased marketplace chamas that aren't already in member list
